...
@section('content')
	...
	<div class="card">
        <div class="card-header">
            <h2>{{ trans('management/role.new.title') }}</h2>
        </div>
        <div class="card-body">
            <form action="{{ URL::route('management.roles.store') }}" method="post"
                ...
                <div class="form-group">
                    <label for="" class="col-sm-2 control-label">{{ trans('model/permission.plural') }}</label>

                    <div class="col-sm-10">
	                    @foreach($permissions as $permission)
		                    <div class="checkbox m-b-15">
			                    <label>
				                    <input type="checkbox" name="permissions[{{ $permission->id }}]" value="{{ $permission->id }}" {{ (array_has(Input::old('roles', []), $permission->id)) ? 'checked' : '' }}>
