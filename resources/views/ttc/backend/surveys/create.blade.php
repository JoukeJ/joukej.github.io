...
@section('content')
	...
	<div class="card">
        <div class="card-header">
            <h2>{{ trans('survey/create.title') }}</h2>
        </div>
        <div class="card-body">
            <form action="{{ URL::route('surveys.store') }}" method="post"
                  class="form-horizontal" role="form">
                <input type="hidden" name="_token" value="{{ csrf_token() }}">

	            <div class="form-group">
		            <label for="" class="col-sm-2 control-label">{{ trans('survey/create.name') }}</label>
	            ...
	            <div class="form-group">
		            <label for="" class="col-sm-2 control-label">{{ trans('survey/create.language') }}</label>

		            <div class="col-sm-10">
			            <div class="fg-line">
				            <select name="language" class="form-control">
                                @foreach(\App\TTC\Common\Helper::getLanguages() as $key => $language)
                                    <option @if($key === Input::old('language')) selected @endif value="{{ $key }}">{{ $language }}</option>
                                @endforeach
                            </select>
			            </div>
		            </div>
                ...
                <div class="form-group">
                    <label for="" class="col-sm-2 control-label">{{ trans('survey/create.start_date') }}</label>
                    <div class="col-sm-10">
                        <div class="fg-line">
                            <input type="text" class="form-control input-sm datetimepicker" id="survey.start_date"
                                   value="{{ Input::old('start_date') }}"
                                   placeholder="{{ trans('survey/create.start_date') }}" name="start_date">
                        </div>
                    </div>
                </div>

                <div class="form-group">
                    <label for="" class="col-sm-2 control-label">{{ trans('survey/create.end_date') }}</label>
                    <div class="col-sm-10">
                        <div class="fg-line">
                            <input type="text" class="form-control input-sm datetimepicker" id="survey.end_date"
                                   value="{{ Input::old('end_date') }}"
                                   placeholder="{{ trans('survey/create.end_date') }}" name="end_date">
                        </div>
                    </div>
                </div>

                <div class="form-group">
                    <label for="" class="col-sm-2 control-label">{{ trans('survey/create.priority') }}</label>
                    <div class="col-sm-10">
                        <div class="fg-line">
                            <input type="text" class="form-control input-sm" id="survey.priority"
                                   value="{{ Input::old('priority') }}"
                                   placeholder="{{ trans('survey/create.priority') }}" name="priority">
                        </div>
                    </div>
                </div>
                        ...
                        <div class="fg-line">
                            <select name="repeat[interval]" id="repeating_survey" class="form-control">
                                ...
                                @foreach(config('ttc.survey.repeating.intervals') as $key => $interval)
                                    <option @if(Input::old('repeating.interval')==$key) selected @endif value="{{ $key }}">{{ $interval }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                ...
                <div class="form-group" id="repeating_end_date" style="display: @if(Input::old('repeating.interval','')!=='') block @else none @endif ;">
                    ...
                    <div class="col-sm-10">
                        <div class="fg-line">
                            <input type="text" class="form-control input-sm datetimepicker" id="repeating.absolute_end_date"
                                   value="{{ Input::old('repeating.absolute_end_date') }}"
                ...
                <div class="form-group">
                    <div class="col-sm-offset-2 col-sm-10">
                        <a href="{{ URL::route('surveys.index') }}"
